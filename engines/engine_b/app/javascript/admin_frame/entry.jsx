...
const NAV_ITEMS = [
  { id: 'dashboard', label: 'Dashboard', icon: cilSpeedometer, section: null },
  { id: 'customers', label: 'Customers', icon: cilPeople, section: 'Workspace' },
  { id: 'projects', label: 'Projects', icon: cilFolderOpen, section: 'Workspace' },
  { id: 'activity', label: 'Activity Log', icon: cilHistory, section: 'System' },
  { id: 'settings', label: 'Settings', icon: cilSettings, section: 'System' },
];
...
const App = () => {
  ...
  const initialPage = (() => {
    const path = window.location.pathname.replace('/v2/adminFrame', '');
  ...
  const breadcrumb = page === 'customers'
    ? [{ label: 'Home', page: 'dashboard' }, { label: 'Customers' }]
    : page === 'projects'
    ? [{ label: 'Home', page: 'dashboard' }, { label: 'Projects' }]
    : page === 'activity'
    ? [{ label: 'Home', page: 'dashboard' }, { label: 'Activity Log' }]
    : page === 'settings'
    ? [{ label: 'Home', page: 'dashboard' }, { label: 'Settings' }]
    : [{ label: 'Home' }, { label: 'Dashboard' }];
  ...
  const navigate = (nextPage, skipHistory = false) => {
    startTransition(() => setPage(nextPage));
    if (!skipHistory) {
      const url = nextPage === 'customers'
        ? '/v2/adminFrame/customers'
        : nextPage === 'projects'
        ? '/v2/adminFrame/projects'
        : nextPage === 'activity'
        ? '/v2/adminFrame/activity'
        : nextPage === 'settings'
        ? '/v2/adminFrame/settings'
        : '/v2/adminFrame';
    ...
    <div>
      ...
      >
        <CSidebarHeader className="border-bottom">
          <CSidebarBrand>
            <img className="sidebar-brand-full" src="/marketing/saas-template-logo.svg" alt="SaaS Template" style={{ height: '32px' }} />
            <img className="sidebar-brand-narrow" src="/marketing/saas-template-icon.svg" alt="ST" style={{ height: '28px', width: '28px' }} />
          </CSidebarBrand>
        </CSidebarHeader>
        <CSidebarNav className="px-2 py-2">
          <CNavItem href="#" active={page === 'dashboard'} onClick={(event) => { event.preventDefault(); navigate('dashboard'); }}>
            <CIcon customClassName="nav-icon" icon={cilSpeedometer} /> Dashboard
          </CNavItem>

          <CNavTitle>Workspace</CNavTitle>
          <CNavItem href="#" active={page === 'customers'} onClick={(event) => { event.preventDefault(); navigate('customers'); }}>
            <CIcon customClassName="nav-icon" icon={cilPeople} /> Customers
            <CBadge className="ms-auto tw-text-[10px] tw-font-semibold tw-rounded-full tw-px-2 tw-py-0.5" color="light" style={{ color: 'var(--app-stone)' }}>
              {navCounts.customers}
            </CBadge>
          </CNavItem>
          <CNavItem href="#" active={page === 'projects'} onClick={(event) => { event.preventDefault(); navigate('projects'); }}>
            <CIcon customClassName="nav-icon" icon={cilFolderOpen} /> Projects
            <CBadge className="ms-auto tw-text-[10px] tw-font-semibold tw-rounded-full tw-px-2 tw-py-0.5" color="light" style={{ color: 'var(--app-stone)' }}>
              {navCounts.projects}
            </CBadge>
          </CNavItem>
          ...
          <CNavItem href="#" active={page === 'activity'} onClick={(event) => { event.preventDefault(); navigate('activity'); }}>
            <CIcon customClassName="nav-icon" icon={cilHistory} /> Activity Log
          </CNavItem>
          <CNavItem href="#" active={page === 'settings'} onClick={(event) => { event.preventDefault(); navigate('settings'); }}>
      ...
      <div className="wrapper d-flex flex-column min-vh-100">
        <CHeader position="sticky" className="mb-0 p-0">
          <CContainer className="border-bottom px-3" fluid>
            <CHeaderToggler onClick={() => setSidebarShow((visible) => !visible)} style={{ marginInlineStart: '-14px' }}>
              <svg width="20" height="20" viewBox="0 0 20 20" fill="none" className="tw-text-slate-500">
                <rect x="2" y="4" width="16" height="1.5" rx="0.75" fill="currentColor" />
                <rect x="2" y="9.25" width="12" height="1.5" rx="0.75" fill="currentColor" />
                <rect x="2" y="14.5" width="8" height="1.5" rx="0.75" fill="currentColor" />
              </svg>
            </CHeaderToggler>
            <div className="ms-3" style={{ position: 'relative', maxWidth: '360px', flex: 1 }}>
              <CIcon
                icon={cilSearch}
                style={{ position: 'absolute', left: '10px', top: '50%', transform: 'translateY(-50%)', color: 'var(--app-stone-light)', zIndex: 2 }}
                size="sm"
              />
              <CFormInput
                placeholder="Customers / Projects / Activity"
                size="sm"
                value={query}
                onChange={(event) => setQuery(event.target.value)}
                style={{ borderRadius: '6px', paddingLeft: '34px', paddingTop: '10px', paddingBottom: '10px', fontSize: '13px' }}
              />
            </div>
            <CDropdown variant="nav-item" alignment="end" className="ms-auto">
              <CDropdownToggle caret={false} className="p-0" style={{ border: 'none', background: 'none' }}>
                <CAvatar size="md" className="tw-bg-[var(--app-accent)] tw-text-white tw-text-[12px] tw-font-semibold tw-cursor-pointer tw-ring-2 tw-ring-[var(--app-stone-ring)] hover:tw-ring-[var(--app-stone-light)] tw-transition-all">
                  {userInitials}
                </CAvatar>
              </CDropdownToggle>
              <CDropdownMenu>
          ...
          <CContainer className="px-3 d-flex align-items-center justify-content-between tw-flex-wrap tw-gap-y-1" fluid>
            <CBreadcrumb style={{ margin: 0, fontSize: '13px' }} className="tw-flex-shrink-0">
              {breadcrumb.map((item, index) => (
                <CBreadcrumbItem
                  key={`${item.label}-${index}`}
                  active={index === breadcrumb.length - 1}
                  href={item.page ? '#' : undefined}
                  onClick={item.page ? (event) => { event.preventDefault(); navigate(item.page); } : undefined}
                >
                  {item.label}
                </CBreadcrumbItem>
              ))}
            </CBreadcrumb>
            <div className="d-flex align-items-center tw-gap-2 tw-min-w-0">
              {loadError && <div className="tw-text-[11px] tw-text-amber-600">{loadError}</div>}
              <div className="tw-text-[12px] tw-font-semibold tw-text-slate-700">{pageTitle}</div>
            </div>
          </CContainer>
        </CHeader>

        <div className="body flex-grow-1">
          <CContainer className="px-3 py-3" fluid>
